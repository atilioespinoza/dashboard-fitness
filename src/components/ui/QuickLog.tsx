...
export function QuickLog({ userId, onUpdate }: { userId: string, onUpdate: () => void }) {
    ...
    const handleLog = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSummary({
                ...
                carbs: payload.carbs,
                fat: payload.fat,
                tdee: existing?.tdee || 2600, // Fallback TDEE
    ...
    return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[2.5rem] p-6 shadow-xl relative overflow-hidden group min-h-[220px]">
            ...
            <AnimatePresence>
                {success && !summary && (
                    <motion.div
                        ...
                        className="absolute inset-0 bg-emerald-500/95 backdrop-blur-sm flex flex-col items-center justify-center text-white p-6 z-20"
                    >
                        <CheckCircle2 size={48} className="mb-2" />
                        <p className="font-black uppercase tracking-widest text-sm text-center">¡Procesado!</p>
                    </motion.div>
                )}
                ...
                {summary && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: 20 }}
                        className="absolute inset-0 bg-white dark:bg-slate-900 z-30 p-5 overflow-y-auto scrollbar-hide"
                    >
                        <div className="flex justify-between items-center mb-4">
                            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400">Resumen Acumulado del Día</h4>
                            <button onClick={() => setSummary(null)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
                        ...
                        <div className="space-y-3">
                            ...
                            {/* Energy Balance */}
                            <div className="mt-4 p-3 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-100 dark:border-white/5">
                                ...
                                <div className="flex justify-between items-end">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[7px] font-black uppercase tracking-tighter text-slate-500">Déficit Promedio Detectado</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
function MetricRow({ label, value, goal, unit, type }: { label: string, value: number, goal: number, unit: string, type: 'max' | 'min' | 'range' }) {
    ...
    return (
        <div className="p-2.5 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-white/5">
